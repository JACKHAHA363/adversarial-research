...
def jacobian_graph(predictions, x, nb_classes):
    ...
    pre-softmax) https://github.com/tensorflow/cleverhans/pull/157 
    actually it can be applied to post-softmax as well
    :param x: the input placeholder
...
def jsma(sess, x, predictions, grads, sample, target, theta, gamma, clip_min,
    ...
    current = utils_tf.model_argmax(sess, x, predictions, adv_x_original_shape, feed)
...
def jsma_batch(sess, x, pred, grads, X, theta, gamma, clip_min, clip_max,
    ...
    :param feed: the extra feed dict
    ...
    for ind, val in enumerate(X):
        ...
        if targets is None:
            ...
            gt = model_argmax(sess, x, pred, val, feed)
        ...
        X_adv[ind], _, _ = jsma(sess, x, pred, grads, val, np.argmax(target),
                                theta, gamma, clip_min, clip_max, feed)
